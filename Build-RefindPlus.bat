...
if exist "%ELF_DLL%" (
  ...
  echo Running GenFw on fixed ELF...
  "GenFw" -e UEFI_APPLICATION -o "%OUTPUT_EFI%" "%FIXED_DLL%"
REM Post-processing to stop the scramble and the Foxconn version error
  echo Fixing identity for old firmware support...
  "%PYTHON_HOME%\python.exe" "%EDK2_DIR%\FixHeader.py" "%OUTPUT_EFI%"
